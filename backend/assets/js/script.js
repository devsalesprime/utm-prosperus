...
// Filtro de busca e data (client-side legado — desabilitado quando server-side está ativo)
const initSearchFilters = () => {
  ...
  // Se qualquer elemento não existir (server-side paginação ativa), sai silenciosamente
  if (!searchInput || !dateFilter) return;

  const filterTable = () => {
    const searchVal = searchInput.value.toLowerCase();
    const dateVal = dateFilter.value;
    const clicksVal = clicksFilter ? clicksFilter.value : '';
    ...
    rows.forEach(row => {
      ...
      if (clicksVal && match) {
        ...
        if (clicksCell) {
          const clicks = parseInt(clicksCell.textContent.trim(), 10);
  ...
  if (clicksFilter) clicksFilter.addEventListener('input', filterTable);
};
...
// ========================================
// PARTE 2.3 - SELETOR DE DOMÍNIO DINÂMICO
// Multi-Domínio Support com Troca de Logo
// ========================================

/**
 * Inicializa o seletor de domínio com comportamento dinâmico
 * - Detecta mudanças de domínio
 * - Troca logo automaticamente (SPA)
...
const initDomainSelector = () => {
  ...
  function updateDomainVisuals(radio) {
    ...
    // Antigravity Design System: Injetar a classe de identidade visual principal no corpo da página
    if (brand === 'Prosperus Club') {
      document.body.classList.remove('theme-sales-prime');
      document.body.classList.add('theme-prosperus-club');
    } else {
      document.body.classList.remove('theme-prosperus-club');
      document.body.classList.add('theme-sales-prime');
    }
    ...
    console.log(`[UTM Generator] Domínio Antigravity ativado: ${brand} (${radio.value})`);
  }
  ...
  if (originalInitTheme) {
    window.initTheme = function(...args) {
      ...
      // Após tema ser alterado, atualizar a logo correspondente
      setTimeout(() => {
...
// Chamar função de inicialização do seletor de domínio
